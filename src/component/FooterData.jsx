...
const FooterData = () => {
  const [email, setEmail] = useState("");
  const [error, setError] = useState("");
  const [boldProducts, setBoldProducts] = useState(new Set());
  const [boldServices, setBoldServices] = useState(new Set());
  const [boldSocial, setBoldSocial] = useState(new Set());
  ...
  const toggleBoldProduct = (id) => {
    const newSet = new Set(boldProducts);
    if (newSet.has(id)) {
      newSet.delete(id);
    } else {
      newSet.add(id);
    }
    setBoldProducts(newSet);
  };

  const toggleBoldService = (id) => {
    const newSet = new Set(boldServices);
    if (newSet.has(id)) {
      newSet.delete(id);
    } else {
      newSet.add(id);
    }
    setBoldServices(newSet);
  };

  const toggleBoldSocial = (id) => {
    const newSet = new Set(boldSocial);
    if (newSet.has(id)) {
      newSet.delete(id);
    } else {
      newSet.add(id);
    }
    setBoldSocial(newSet);
  };
          ...
          <ul className="">
            {ProjectLinks.map((item) => (
              <li key={item.id} className="flex items-start gap-3 py-1">
                <a
                  href={item.link}
                  onClick={() => toggleBoldProduct(item.id)}
                  className="text-primary text-4xl lg:text-5xl text-start"
                >
                  <p className={`text-white font-lato ${boldProducts.has(item.id) ? "font-bold" : ""}`}>
                    {item.title}
                  </p>
                </a>
              </li>
          ...
          <ul>
            {ServiceLinks.map((item) => (
              <li key={item.id} className="flex items-start gap-3 py-1">
                <a
                  href={item.link}
                  onClick={() => toggleBoldService(item.id)}
                  className=""
                >
                  <p className={`text-white font-Lato ${boldServices.has(item.id) ? "font-bold" : ""}`}>
                    {item.title}
                  </p>
                </a>
              </li>
            ...
            <ul className="flex items-center justify-center text-center space-x-6 font-lato">
              {Socialdata.map((item) => (
                <a
                  ...
                  aria-label="Chat on WhatsApp"
                  key={item.id}
                  onClick={() => toggleBoldSocial(item.id)}
                  className={`lg:text-3xl text-2xl bg-white text-[#696969] rounded-full p-1 ${boldSocial.has(item.id) ? "font-bold" : ""}`}
                >
                  {item.icon}
                </a>
              ))}
